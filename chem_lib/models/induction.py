...
class Induction(nn.Module):
    def __init__(self, C, S, H, iterations):
        ...
        self.C = C
        self.S = S
        self.H = H #  2 * hidden_size
        self.iterations = iterations
        self.W = torch.nn.Parameter(torch.randn(H, H)) #(256, 256)

    def forward(self, x):
        b_ij = torch.zeros(self.C, self.S).to(x) # b_ij.size([2,10])
        for _ in range(self.iterations):  # self.iterations is 3
            d_i = F.softmax(b_ij.unsqueeze(2), dim=1)  # (C,S,1) # (2,10,1) 
            # x.reshape(-1, self.H).size([10,256])
            e_ij = torch.mm(x.reshape(-1, self.H), self.W).reshape(self.C, self.S, self.H)  # (C,S,H) (2,10,300)
            c_i = torch.sum(d_i * e_ij, dim=1)  # (C,H) # (2,300)
            # squash 
            # torch.sum(c_i ** 2, dim=1).size([1,2]) reshape is ([2,1])
            squared = torch.sum(c_i ** 2, dim=1).reshape(self.C, -1)
            coeff = squared / (1 + squared) / torch.sqrt(squared + 1e-9) # coeff.size([2,1])
            c_i = coeff * c_i # torch.size([2, 256])
            c_produce_e = torch.bmm(e_ij, c_i.unsqueeze(2))  # (C,S,1) (2, 5, 1)
...
class FewShotInduction(nn.Module):
    ...
    def forward(self, x):
        # x.size([64,510]) 64 data, 510 dim 
        # support_encoder.size([10,256]) query_encoder.size([54, 256])
        support_encoder, query_encoder = self.encoder(x)  # (k*c, 2*hidden_size)
        class_vector = self.induction(support_encoder) # size = ([2, 256])
